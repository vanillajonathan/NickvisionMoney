...
bool Account::exportAsPDF(const std::string& path) const
{
    ...
    //Get Source Sans Pro Font Path
    std::string pathToSSPFont;
    if(std::filesystem::exists("/usr/share/org.nickvision.money/sourcesanspro.ttf"))
    {
        pathToSSPFont = "/usr/share/org.nickvision.money/sourcesanspro.ttf";
    }
    else if(std::filesystem::exists("/app/share/org.nickvision.money/sourcesanspro.ttf"))
    {
        pathToSSPFont = "/app/share/org.nickvision.money/sourcesanspro.ttf";
    }
    else
    {
        pathToSSPFont = "sourcesanspro.ttf";
    }
    //Write PDF
    PDFDocument pdf;
    //Source Sans Pro Font
    std::string sourceSansProFont{ pdf.loadTTFontFromFile(pathToSSPFont) };
    if(sourceSansProFont.empty())
    {
        return false;
    }
    //First Page
    ...
    //First Page - Overview
    page1.setUTFFont(sourceSansProFont, 9);
